...
def caption_generator(image_id, image_id_to_class_id, class_id_to_name_clean, image_id_to_attributes, attribute_id_to_name) -> list[str]:
    ...
    for idx in attr_template_idxs:
        ...
        for img_attr in selected_image_attributes:
            ...
            img_attr = ' '.join(key.split('_')[1:])
            img_attr = f'has {val} {img_attr}'
            final_image_attributes.append(img_attr)
        temp = WITH_ATTR_TEMPLATES[idx]
        attrs = ' and '.join(final_image_attributes)
        attrs = attrs.split()
        attrs = ' '.join(attrs[1:])
        temp = temp.format(cls=class_name, attrs=attrs)
        captions.append(temp)
    
    return captions

def create_csv(file_name, selected_class_ids, image_id_to_path, image_id_to_class_id, class_id_to_name_clean, image_id_to_attributes, attribute_id_to_name):
...
def main():
    ...
    num_classes = 3
    selected_class_ids = set(random.sample(range(1, 201), num_classes))
    create_csv('cub_finetune_3cls.csv', selected_class_ids, image_id_to_path, image_id_to_class_id, class_id_to_name_clean, image_id_to_attributes, attribute_id_to_name)
